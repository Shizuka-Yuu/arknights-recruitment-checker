...
Arknights（アークナイツ）の公開求人でオペレーターを検索し、確定組み合わせを確認するためのWebツールです。

## 🎯 特徴

- **リアルタイム検索**: タグ選択で即座にオペレーターを絞り込み
- **確定組み合わせ表示**: 1〜5タグ選択で★4以上確定またはロボット確定の組み合わせを表示
- **全組み合わせ分析**: 選択したタグから可能なすべての組み合わせを網羅的に表示
- **候補ラベル表示**: 確定対象外のオペレーターに「候補」ラベルを表示（低レアリティ非表示機能付き）
- **ロボット説明**: ロボット確定の場合に特別な説明を表示（現在は未使用）
- **多言語対応**: 日本語と英語のUIに対応
- **テーマ切り替え**: ライトモードとダークモードをサポート
- **Wiki連携**: オペレーター名/アイコンクリックでアークナイツ攻略 Wiki（日本語）へ移動
...
1. **タグを選択**: オペレーターの特性に合わせてタグを選択（最大5個）
2. **検索結果確認**: 選択したタグに一致するオペレーターが表示
3. **確定組み合わせ**: 1〜5タグ選択時は確定組み合わせと対象オペレーターを表示
4. **全組み合わせ分析**: 可能なすべてのタグ組み合わせと結果を確認
5. **Wikiアクセス**: オペレーターのアイコンをクリックで詳細情報へ
...
A web tool for searching operators and checking guaranteed combinations in Arknights' public recruitment.

## 🎯 Features

- **Real-time Search**: Instantly filter operators by selecting tags
...
- **Candidate Labels**: Display "Candidate" labels for operators outside guaranteed targets (with low-rarity hide feature)
- **Robot Explanations**: Show special explanations for robot guaranteed cases (currently unused)
- **Multi-language Support**: Japanese and English UI support
- **Theme Switching**: Light mode and dark mode support
- **Wiki Integration**: Click operator names/icons to navigate to Arknights Strategy Wiki (Japanese)
...
1. **Select Tags**: Choose tags based on operator characteristics (max 5 tags)
2. **Check Search Results**: View operators matching selected tags
3. **Guaranteed Combinations**: Display guaranteed combinations and target operators with 1-5 tags
4. **Complete Combination Analysis**: Check all possible tag combinations and results
5. **Wiki Access**: Click operator icons for detailed information
...
- **Dictionary Function**: Multi-language support for operator names, tag names, UI text
...
- Multi-language operator name display
...
- **Candidate Labels**: Display "Candidate" labels for operators outside guaranteed targets (with low-rarity hide feature)
- **Robot Explanations**: Show special explanations for robot guaranteed cases (currently unused)
- **System Messages**: Clearly display guarantee status when tags are selected

### Image Processing
- **Multiple Path Support**: Support for multiple file formats of operator icons
...
│   ├── CharacterGrid.tsx    # Operator list display
...
├── hooks/              # Custom hooks
│   ├── useCharacters.ts      # Operator data fetching
