...
export const AllCombinationResults: React.FC<AllCombinationResultsProps> = ({ 
  ...
  if ((!allCombos || allCombos.length === 0) && (!guaranteedResults || guaranteedResults.length === 0)) {
    return (
      <div className="text-center py-8 text-gray-500 dark:text-gray-400">
        {language === 'ja' ? '該当する組み合わせが見つかりません' : 'No combinations found'}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
                    ...
                    {!result.combo.includes('上級エリート') && result.characters.some(char => char.tags.includes('ロボット')) && (
                      <div className="flex flex-col items-start" style={{ display: 'block', width: '100%' }}>
                        ...
                        <div className="text-xs mt-1" style={{ color: '#f59e0b' }}>
                          {language === 'ja' ? '▼短時間はタグ消失リスク' : '▼Short time has tag loss risk'}
                        </div>
                      </div>
                    )}
                  </div>
      ...
      {/* その他の全ての組み合わせ結果 - キャラクター数でソートして表示 */}
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-1 text-xs rounded" style={{ backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-secondary)' }}>
            {allCombos.length} {language === 'ja' ? '組み合わせ' : 'combinations'}
          </span>
        </div>
        ...
        {allCombos.length > 5 && (
          ...
          >
            {showAllCombos 
              ? (language === 'ja' ? `▲ 折りたたむ (${allCombos.length}件中5件表示)` : `▲ Show less (${allCombos.length} combinations, showing 5)`)
              : (language === 'ja' ? `▼ もっと見る (${allCombos.length - 5}件残り)` : `▼ Show more (${allCombos.length - 5} remaining)`)}
          </button>
        )}
      </div>
    </div>
